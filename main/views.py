...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if customer.is_valid():
            ...
            newuser.pix=pix
            newuser.save()
            messages.success(request, f'dear {user}your account is created successfuly')
            return redirect('home')
        else:
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if passupdate:
            ...
            messages.success(request, f'your password is updated sucessfully')
            return redirect('profile')
        else:
            messages.error(request, f'the following errors where detected: {passupdate.errors}')
...
def add_to_cart(request):
        ...
        if cart:
            basket = Cart.objects.filter(user__username= request.user.username, products = product.id, price =product.price,  quantity = quantity).first()
            if basket:
                basket.quantity += quantity
                basket.amount= product.price * basket.quantity
                basket.save()
                messages.success(request, 'one item added cart')
                return redirect('home')
            else:
                newitem = Cart()
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_61dca06da2e9d9e15ea6bc2e8076c97d4a5df3a6'#secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize'#paystack call url
        cburl = 'http://13.53.205.172/thankyou'
        ref = str(uuid.uuid4())#reference id required by paystack as an additional refrence number
        order_no = profile.id 
        ...
        data ={'refrence':ref, 'amount':int(amount), 'email':email, 'callback_url':cburl, 'order_number':order_no, 'currency':'NGN'}


        #make a cal to paystack
        try:
